 ...
 * @date 2019-07-27
 */
public class NO5036_MinimumCostTest {
...
//        int[][] connections={{2,1,50459},{3,2,47477},{4,2,52585},{5,3,16477}};
        ...
        //
        System.out.println(minimumCost(num, connections));
    }


    public static int minimumCost(int num, int[][] connetions) {
        int sum = 0;
        ...
        Arrays.sort(connetions, new Comparator<int[]>() {
            @Override
        ...
        for (int[] connection : connetions) {

            int pFrom = getParent(connection[0], parents);
            int pTo = getParent(connection[1], parents);
    ...
    private static int getParent(int son, int[] parents) {
        if (parents[son] == son) {
            return son;
        } else {
            return getParent(parents[son], parents);
        }
    }
